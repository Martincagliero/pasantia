...
function ProfilesScreen() {
  ...
  return (
    <div className="w-full space-y-2 overflow-hidden bg-[#eef1f5] px-3 py-3 text-slate-800"><h4 className="text-[13px] font-bold">Explorar perfiles</h4><div className="flex gap-1 text-[6px]"><span className="rounded-full bg-brand-500 px-2 py-1 text-white">Estudiantes</span><span className="rounded-full bg-white px-2 py-1">Empresas</span><span className="rounded-full bg-white px-2 py-1">Embajadores</span></div>{people.map(([avatar,name,study,skills]) => <article key={name} className="w-full min-w-0 overflow-hidden rounded-xl border bg-white p-2 shadow-sm"><div className="flex min-w-0 gap-2"><img src={avatar} alt={name} className="h-7 w-7 shrink-0 rounded-full object-cover" /><div className="min-w-0 flex-1"><h5 className="truncate text-[9px] font-bold">{name}</h5><p className="line-clamp-2 text-[6px] leading-relaxed text-slate-400">{study}</p></div></div><div className="mt-1.5 flex min-w-0 flex-wrap gap-1">{skills.split(' · ').map((skill) => <span key={skill} className="max-w-full truncate rounded-full border bg-slate-50 px-2 py-0.5 text-[6px] text-slate-500">{skill}</span>)}</div><p className="mt-1 text-[6px] text-slate-400">Ver perfil →</p></article>)}
    </div>
  );
}
