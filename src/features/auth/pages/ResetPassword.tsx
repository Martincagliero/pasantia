  ...
  if (!authorized) {
    return (
      <AuthStatusCard icon={<ShieldAlert className="h-7 w-7" />} title="Acceso no válido" message="Abrí esta pantalla desde el enlace que enviamos a tu email. El enlace puede haber vencido.">
        <Link to="/recuperar-password" className="mt-6 inline-flex rounded-xl bg-brand-500 px-5 py-3 text-sm font-semibold text-white hover:bg-brand-400">Solicitar otro enlace</Link>
      </AuthStatusCard>
    );
  }
  if (complete) {
    return (
      <AuthStatusCard icon={<CheckCircle2 className="h-7 w-7" />} title="Contraseña actualizada" message="Cerramos todas las sesiones de tu cuenta. Ya podés ingresar con tu contraseña nueva.">
        <Link to="/ingresar" className="mt-6 inline-flex rounded-xl bg-brand-500 px-5 py-3 text-sm font-semibold text-white hover:bg-brand-400">Ingresar de nuevo</Link>
      </AuthStatusCard>
    );
  }
  ...
  return (
    <AuthStatusCard title="Creá una contraseña nueva" message="Debe ser única y tener al menos 12 caracteres, mayúscula, minúscula, número y símbolo.">
      <form onSubmit={handleSubmit} className="mt-7 space-y-4 text-left">
        ...
        <button type="submit" disabled={loading} className="w-full rounded-xl bg-brand-500 py-3.5 text-[15px] font-semibold text-white hover:bg-brand-400 disabled:opacity-60">
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2.5 rounded-xl border border-white/10 bg-[#12151E] px-3.5 focus-within:border-brand-400/60">
        <Lock className="h-[18px] w-[18px] shrink-0 text-white/40" />
        ...
        <button type="button" onClick={onToggle} aria-label={visible ? 'Ocultar contraseña' : 'Mostrar contraseña'} className="text-white/40 hover:text-white">
